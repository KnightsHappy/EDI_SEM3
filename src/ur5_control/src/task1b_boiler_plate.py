...
******************************************************************************************
*
* [TASK 1C] - Arm Manipulation with Servoing (Fixed Version + P2 TF)
* Author: Nihit Shevade
*
* Description:
* Arm moves: Start → P1 → P3 → Start → P2_modified,
* using PD-based Cartesian servoing.
* Added TF broadcasting to visualize P2_modified in RViz.
*
******************************************************************************************
"""
...
class ArmWaypointController(Node):
    def __init__(self):
        ...
        self.tf_broadcaster = TransformBroadcaster(self)  # <-- TF broadcaster for visualization
    ...
    def control_loop(self):
        # --- Publish P2_modified transform for visualization ---
        self.publish_p2_transform()
    ...
    # --- NEW METHOD TO PUBLISH P2 TF ---
    def publish_p2_transform(self):
        t = TransformStamped()
        t.header.stamp = self.get_clock().now().to_msg()
        t.header.frame_id = 'base_link'       # parent frame
        t.child_frame_id = 'p2_target'        # visualization frame

        # Position
        t.transform.translation.x = float(self.p2_modified['pos'][0])
        t.transform.translation.y = float(self.p2_modified['pos'][1])
        t.transform.translation.z = float(self.p2_modified['pos'][2])

        # Orientation
        t.transform.rotation.x = float(self.p2_modified['quat'][0])
        t.transform.rotation.y = float(self.p2_modified['quat'][1])
        t.transform.rotation.z = float(self.p2_modified['quat'][2])
        t.transform.rotation.w = float(self.p2_modified['quat'][3])
